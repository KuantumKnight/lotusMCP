...
def test_mandatory_and_excluded_lite_tools():
    ...
    # read-only artifact fetch + advisory technique suggestions are safe for LITE
    assert "kb_artifact" in lite, "kb_artifact (read-only) should be in LITE"
    assert "technique_suggest" in lite, "technique_suggest (advisory) should be in LITE"
    # exec / ops / scrape / submit / library must never leak into the read-only LITE client
    for t in ("session_edit_run", "session_close", "session_list",
              "case_compact", "case_gc", "case_metrics", "propose_and_run",
              "lotus_submit", "technique_promote"):
